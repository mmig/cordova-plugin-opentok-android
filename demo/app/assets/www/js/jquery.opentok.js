...
(function ($) {
    ...
    $.openTok = function (options, element) {
        ...
        if (arguments.length) {
            ...
            var self = this;
            this.element.bind("remove.openTok", function (e) {
                self.destroy(e);
            });
            this._init();
        }
    ...
    $.openTok.prototype = {
        ...
        options: {
            ...
            idPrefix: "ot",
            streamWrapper: '<div class="opentok-stream"></div>'
        },
        instances: {},
        status: {
            connected: false,
        ...
        _addHtmlEventListeners: function (session) {
            ...
            session.addEventListener("sessionConnected", function (e) {
              try{
                ...
                console.debug('auto-publish: ' + self.options.autoPublish);
                ...
                if (self.options.autoPublish) {
            	  ...
            	  console.error('ERROR on handling connecting to session: ' + msg);
              }
            });
        ...
        _addStream: function (stream) {
            ...
            streamWrapperContainer.on("remove", function (event) {//FIXME add remove handler
                console.error("Element was removed "+this.id+ ", calltrace: "+(new ReferenceError('calltrace-removeSubscriberWrapper')).stack);//TODO trigger remove for subscriber view on OpenTok cordova plugin
            });
            
            var streamOptions = {
                subscribeToAudio: !this.status.pushTalking,
                width: $('.opentok-streams').width(),
                height: $('.opentok-streams').height()
            };
        ...
        _removeStream: function (stream) {
            if (this.streams[stream.streamId]) {
                ...
                $('#callHangup, .callHangup').trigger('click');
            }
            ...
            try {
                if (this.status.connected && this.session.capabilities.publish) {
                    ...
                    var publishProperties = $.extend({
                        publishAudio: true,
                        width: this.css.jq.publisherContainer.width(),
                        height: this.css.jq.publisherContainer.height()
